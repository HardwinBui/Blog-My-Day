...
export default function ViewPost() {
  ...
  useEffect(() => {
    async function getPostData() {
      const responsePost = await fetch(`http://blogmydaybackend.onrender.com/post/${params.id}`);
      ...
      // Set post data
      var posts = await responsePost.json();
      setPosts(posts);

      var date = new Date(postInfo.date_created);
      ...
      // Verify user
      if (!isLoading) {
        var verifyPost = false;
        if (isAuthenticated)
          verifyPost = postInfo.user == user.nickname.toString();
        verifyPost = verifyPost && isAuthenticated;
        setUser(verifyPost);
      }
    }
  ...
  async function deletePostAPI() {
    await fetch(`http://blogmydaybackend.onrender.com/blog/${postInfo.blogID}`).then(async response => {
      const blogInfo = await response.json();

      var editedBlog = { ...blogInfo };
      editedBlog.posts = editedBlog.posts.filter(e => e !== params.id);
      await fetch(`http://blogmydaybackend.onrender.com/blog/update/${postInfo.blogID}`, {
        method: "POST",
    ...
    await fetch(`http://blogmydaybackend.onrender.com/post/delete/${params.id}`, {
      method: "DELETE"
    });
  ...
  async function toggleLikePost() {
    ...
    if (editedPost.likes['1'].includes(user.nickname)) {
      editedPost.likes['1'] = editedPost.likes['1'].filter(e => e !== user.nickname);
    }
    else {
      if (editedPost.likes['-1'].includes(user.nickname))
        editedPost.likes['-1'] = editedPost.likes['-1'].filter(e => e !== user.nickname);
  ...
  function viewBlog() {
    return (
      <Link to={`/viewBlog/${postInfo.blogID}`}>
  ...
  function PostLikes() {
    return postInfo.likes !== undefined &&
      <>
        <div>
          <Link onClick={() => { toggleLikePost(); }}>
            <button class="like-icon">
              {!postInfo.likes['1'].includes(user.nickname) &&
        ...
        <div>
          <h5 class="like-value">  {postInfo.likes['1'].length - postInfo.likes['-1'].length}  </h5>
        </div>

        <div>
          <Link onClick={() => { toggleDislikePost(); }}>
            <button class="like-icon">
              {!postInfo.likes['-1'].includes(user.nickname) &&
  ...
  return (
    <div class="page-container">
      ...
      <div class="info-container">
        ...
        <div><h5>
          {postInfo.comments !== undefined && postInfo.comments.length + ' '}
          {postInfo.comments === undefined && '0 '}
          comment
          {postInfo.comments !== undefined && postInfo.comments.length !== 1 && 's'}
          {postInfo.comments === undefined && 's'}
        </h5></div>
        <div><p></p></div>
      </div>
